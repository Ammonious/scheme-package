...
// vertice of the MaterialSegmentedControl's Rect.
//
...
/// See also:
///
///  * [MaterialSegmentedControl], a segmented control widget in the
...
class _SegmentedControlState<T> extends State<MaterialSegmentedControl<T>>
  ...
  void _updateTabController() {
    ...
    if (_controllerIsValid) {
      tabController.animation.removeListener(_handleTabControllerAnimationTick);
      tabController.removeListener(_handleTabControllerTick);
    }
    _controller = newController;
    if (_controller != null) {
      _controller.animation.addListener(_handleTabControllerAnimationTick);
      _controller.addListener(_handleTabControllerTick);
      _highlighted = _controller.index as T;
    }
  }
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller.indexIsChanging) {
      ...
      _currentIndex = _controller.index as T;
      _highlighted = _currentIndex;
      tabController.animateTo(_controller.index == 0 ? 1 : 0);
      _animateHighlightController(
          at: _highlighted, forward: _controller.index == 0);
    }
  }
  ...
  void didChangeSelectedViaGesture() {
    tabController.animateTo(_highlighted as int);
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.all(
                  Radius.circular(widget.borderRadius ?? _kCornerRadius)),
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  void performLayout() {
    ...
    double childWidth =
        width ?? (constraints.minWidth - totalSeparatorWidth) / childCount;
    double maxHeight = height ?? _kMinSegmentedControlHeight;
  ...
  void _paintThumb(PaintingContext context, Offset offset, Rect thumbRect) {
    ...
    List<BoxShadow> _shadow = boxShadow ?? thumbShadow;
    final RRect thumbRRect = RRect.fromRectAndRadius(thumbRect.shift(offset),
        borderRadius != null ? Radius.circular(borderRadius) : _kThumbRadius);

    for (final BoxShadow shadow in _shadow) {
      context.canvas
